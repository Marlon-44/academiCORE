...
class PensumIntegrationTest {
    ...
    @Autowired private CrearUniversidadUseCase crearUniversidadUseCase;
    @Autowired private CrearFacultadUseCase crearFacultadUseCase;
    @Autowired private CrearDepartamentoAcademicoUseCase crearDepartamentoUseCase;
    @Autowired private CrearProgramaAcademicoUseCase crearProgramaUseCase;
    @Autowired private CrearPlanEstudiosUseCase crearPlanEstudiosUseCase;
    @Autowired private CambiarEstadoPensumUseCase cambiarEstadoUseCase;
    @Autowired private AgregarNivelCurricularUseCase agregarNivelUseCase;
    @Autowired private RegistrarAsignaturaUseCase registrarAsignaturaUseCase;
    @Autowired private AgregarAsignaturaAPlanUseCase agregarAsignaturaAPlanUseCase;
    @Autowired private DefinirRequisitoUseCase definirRequisitoUseCase;
    @Autowired private ConsultarMallaCompletaUseCase consultarMallaUseCase;
    ...
    private PlanEstudiosResponse crearFlujoCompleto() {
        ...
        FacultadResponse fac = crearFacultadUseCase.execute(
                CrearFacultadRequest.builder().universidadId(univ.getId()).codigo("F001").nombre("Facultad de Ingeniería").build());

        DepartamentoResponse dep = crearDepartamentoUseCase.execute(
                CrearDepartamentoRequest.builder().facultadId(fac.getId()).codigo("D001").nombre("Departamento de Sistemas").build());

        ProgramaResponse prog = crearProgramaUseCase.execute(
                CrearProgramaRequest.builder().facultadId(fac.getId()).codigoInterno("P001").codigoSnies("12345")
        ...
        return crearPlanEstudiosUseCase.execute(
                CrearPlanEstudiosRequest.builder().programaId(prog.getId()).periodicidadId(1L)
                        .codigoPensum("2026").nombre("Pensum 2026").fechaVigenciaDesde(LocalDate.of(2026, 1, 1))
    ...
    void prerrequisitoDebeEstarEnNivelAnteriorOIgual() {
        ...
        AsignaturaResponse asig1 = registrarAsignaturaUseCase.execute(
                RegistrarAsignaturaRequest.builder().departamentoId(1L).codigo("PROG1").nombre("Programación I").build());
        AsignaturaResponse asig2 = registrarAsignaturaUseCase.execute(
                RegistrarAsignaturaRequest.builder().departamentoId(1L).codigo("PROG2").nombre("Programación II").build());

        PlanAsignaturaResponse pa1 = agregarAsignaturaAPlanUseCase.execute(plan.getId(),
                AgregarAsignaturaAPlanRequest.builder().asignaturaId(asig1.getId()).nivelCurricularId(nivel1.getId())
                        .tipoAsignaturaId(1L).creditos(new BigDecimal("3.00"))
                        .horasDocenciaSemanales(new BigDecimal("2.00")).horasTrabajoIndependiente(new BigDecimal("4.00"))
                        .esHabilitable(true).build());

        PlanAsignaturaResponse pa2 = agregarAsignaturaAPlanUseCase.execute(plan.getId(),
                AgregarAsignaturaAPlanRequest.builder().asignaturaId(asig2.getId()).nivelCurricularId(nivel2.getId())
                        .tipoAsignaturaId(1L).creditos(new BigDecimal("3.00"))
                        .horasDocenciaSemanales(new BigDecimal("2.00")).horasTrabajoIndependiente(new BigDecimal("4.00"))
                        .esHabilitable(true).build());

        assertDoesNotThrow(() -> definirRequisitoUseCase.execute(plan.getId(), pa2.getId(),
                DefinirRequisitoRequest.builder().planAsignaturaRequeridaId(pa1.getId()).tipoRequisitoId(1L).build()));

        BusinessException ex = assertThrows(BusinessException.class, () ->
                definirRequisitoUseCase.execute(plan.getId(), pa1.getId(),
                        DefinirRequisitoRequest.builder().planAsignaturaRequeridaId(pa2.getId()).tipoRequisitoId(1L).build()));
    ...
    void flujoCompletoCrearPensumYMalla() {
        ...
        AsignaturaResponse asig = registrarAsignaturaUseCase.execute(
                RegistrarAsignaturaRequest.builder().departamentoId(1L).codigo("MAT101").nombre("Matemáticas I").build());

        agregarAsignaturaAPlanUseCase.execute(plan.getId(),
                AgregarAsignaturaAPlanRequest.builder().asignaturaId(asig.getId()).nivelCurricularId(nivel1.getId())
                        .tipoAsignaturaId(1L).creditos(new BigDecimal("4.00"))
                        .horasDocenciaSemanales(new BigDecimal("3.00")).horasTrabajoIndependiente(new BigDecimal("6.00"))
                        .esHabilitable(true).build());
